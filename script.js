...
function hideRandomWords(lyrics) {
  ...
  const indicesToHide = [];
  const hiddenWords = [];
  const numWordsToHide = Math.max(3, Math.floor(words.length * 0.2)); // 20% of words, minimum 3
  ...
  indicesToHide.forEach((index, i) => {
    ...
    words[index] = `<input type="text" id="blank_${i}" class="hidden-word" />`; // Use the "hidden-word" class for better styling
  });
